...
public class PoisManager {
    ...
    public func setOrCreateIconPoi(id: String, icon: UIImage, iconSize: MarkerIconSize?) {
        if pois[id] != nil {
            ...
            pois[id] = Poi(
                id: id, icon: icon, iconSize: iconSize, markerPois: [], handler: markerHandler)
...
struct Poi {
    ...
    public init(
        id: String, icon: UIImage, iconSize: MarkerIconSize?, markerPois: [MarkerIconPoi],
        handler: IconLayerHander
    ) {
        self.id = id
    ...
    mutating func setMarkers(markerPois: [MarkerIconPoi]) {
        ...
        let nMarkersPoi = markerPois.map({
            let config = $0.configuration.copyWith(
                icon: markerIcon.icon, iconSize: markerIcon.size, angle: nil, anchor: nil)
    ...
    mutating func setIconMarker(icon: UIImage, size: MarkerIconSize?) {
        ...
        self.markerPois.enumerated().forEach { index, poi in
            let config = poi.configuration.copyWith(
                icon: icon, iconSize: size, angle: nil, anchor: nil)
...
public struct MarkerIconPoi {
    ...
    public init(location: CLLocationCoordinate2D, angle: Float?, anchor: MarkerAnchor?) {
        self.configuration = MarkerConfiguration(
            icon: UIImage(), iconSize: nil, angle: angle, anchor: anchor)
        self.location = location
    }
    func createMapIcon() -> MCIconInfoInterface? {
        let texture = configuration.icon.toTexture(angle: configuration.angle ?? 0)
        let iconSize =
            if let iconSize = configuration.iconSize {
        ...
        if configuration.anchor != nil {
            return MCIconFactory.createIcon(
                withAnchor: location.id(),
                coordinate: mcCoord,
                texture: texture,
                iconSize: iconSize,
                scale: configuration.scaleType.getValue(), blendMode: .NORMAL,
                iconAnchor: MCVec2F(
                    x: Float(configuration.anchor!.x), y: Float(configuration.anchor!.y)))
        }
        return MCIconFactory.createIcon(
            location.id(),
            coordinate: mcCoord,
