...
public class CircleOSM: Shape {
    ...
    public init(
        center: CLLocationCoordinate2D, distanceInMeter: Double, style: ShapeStyleConfiguration
    ) {
        self.center = center
...
extension CircleOSM {
    func toCoordShape(lengthInMeters: Double) -> [CLLocationCoordinate2D] {
        ...
        for _angle in angles {
            let onCircle = center.destinationPoint(
                distanceInMeter: lengthInMeters, bearingInDegree: Double(_angle))
    ...
    func createBorderShape(id: String) -> MCLineInfoInterface? {
        ...
        return MCLineFactory.createLine(
            "\(id)-border",
            coordinates: bordersMCCoords,
            style: MCLineStyle(
                color: MCColorStateList(
                    normal: style.borderColor.mapCoreColor,
